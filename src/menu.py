...
def titulo_jogo(tela):
    ...
    menu_titulo = tela.blit(
        menu_titulo,
        (400 - menu_titulo.get_width() // 2, 150 - menu_titulo.get_height() // 2),
    )
    return menu_titulo
...
def titulo_game_over(tela):
    ...
    menu_titulo = tela.blit(
        menu_titulo,
        (400 - menu_titulo.get_width() // 2, 200 - menu_titulo.get_height() // 2),
    )
    return menu_titulo
...
def creditos():
    ...
    menu_fonte_creditos.render("Pai Games Inc", True, (WHITE))
...
def imprime_highscores(tela, screen):
    ...
    for i in range(limit):
        ...
        tela.blit(
            score, (screen.dimensoes[0] / 2 - score.get_width() // 2, 160 + 30 * i)
        )
...
def imprime_creditos(tela, screen):
    ...
    for i in range(len(alunos)):
        ...
        tela.blit(
            aluno, (screen.dimensoes[0] / 2 - aluno.get_width() // 2, 260 + 30 * i)
        )


def abre_tela_inicial(tela, screen):
    menu_opcoes = (
        escreve_texto("PLAY GAME", 28),
        escreve_texto("HIGH SCORES", 28),
        escreve_texto("NEW GAME", 28),
        escreve_texto("CRÉDITOS", 28),
    )

    botao_high_scores_voltar = tela.blit(
        escreve_texto("NEW GAME", 28),
    ...
    botao_play_game = tela.blit(
        menu_opcoes[0], (screen.dimensoes[0] / 2 - menu_opcoes[0].get_width() // 2, 300)
    )
    botao_high_scores = tela.blit(
        menu_opcoes[1], (screen.dimensoes[0] / 2 - menu_opcoes[1].get_width() // 2, 340)
    )

    botao_creditos = tela.blit(
        menu_opcoes[3], (screen.dimensoes[0] / 2 - menu_opcoes[3].get_width() // 2, 380)
    )
...
def menu_game(tela, screen):
    ...
    while menu_ativo:
        ...
        if tela_inicial[4].collidepoint(pos) & pressed1 == 1:
            ...
            botao_voltar = tela.blit(
                empresa, (screen.dimensoes[0] / 2 - empresa.get_width() // 2, 540)
            )
...
def game_over(tela, screen, pontos):
    ...
    tela.blit(
        digite_seu_nome,
        (screen.dimensoes[0] / 2 - digite_seu_nome.get_width() // 2, 280),
    )
        ...
        for event in pygame.event.get():
            ...
            if inp != "" and not scores:
                ...
                botao_salvar = tela.blit(
                    texto_botao_salvar,
                    (
                        screen.dimensoes[0] / 2 - texto_botao_salvar.get_width() // 2,
                        440,
                    ),
                )
            ...
            if botao_salvar.collidepoint(pos) and (event.type == pygame.MOUSEBUTTONUP):
                ...
                botao_voltar = tela.blit(
                    escreve_texto("NEW GAME", 28),
                    (
                        screen.dimensoes[0] / 2 - texto_botao_voltar.get_width() // 2,
                        540,
                    ),
                )
